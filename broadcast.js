...
export async function broadcastPendingPool(pool) {
  ...
  try {
    ...
    try {
      ...
      try {
        await signer.callStatic.sendTransaction(txReq);
      } catch (dryErr) {
        await emitTradeAlert("skip", pool, `Dry run failed | ${dryErr.message}`, { venue: "Aave" });
        return { status: "skip", reason: "dry_run_failed" };
      }

      const gasObj = await getDynamicGas(txReq, 150); // boost priority
      if (!gasObj) {
        await emitTradeAlert("skip", pool, "Aave gas estimate failed", { venue: "Aave" });
      } else {
        ...
        const gasCostUsd = computeGasCostUsd(txReq.gasLimit, txReq.maxFeePerGas, gasTokenUsd);
        if (gasCostUsd > estProfit) {
          await emitTradeAlert("skip", pool, "Aave gas>profit", { venue: "Aave" });
        } else {
          txReq.nonce = await getNextNonce();
          try {
            ...
            await emitTradeAlert("fail", pool, `Aave send failed | ${sendErr.message}`, { venue: "Aave" });
          }
        }
      }
    } catch (e) {
    ...
    try {
      ...
      try {
        ...
        await emitTradeAlert("skip", pool, `Dry run failed | ${dryErr.message}`, { venue: "Balancer" });
        return { status: "skip", reason: "dry_run_failed" };
      }

      const gasObj = await getDynamicGas(txReq, 150); // boost priority
      if (!gasObj) {
        await emitTradeAlert("skip", pool, "Balancer gas estimate failed", { venue: "Balancer" });
      } else {
        txReq.gasLimit = gasObj.gasLimit;
        txReq.maxFeePerGas = gasObj.maxFeePerGas;
        txReq.maxPriorityFeePerGas = gasObj.maxPriorityFeePerGas;
        ...
        const gasCostUsd = computeGasCostUsd(txReq.gasLimit, txReq.maxFeePerGas, gasTokenUsd);
        if (gasCostUsd > estProfit) {
          await emitTradeAlert("skip", pool, "Balancer gas>profit", { venue: "Balancer" });
        } else {
          txReq.nonce = await getNextNonce();
          try {
            ...
            await emitTradeAlert("fail", pool, `Balancer send failed | ${sendErr.message}`, { venue: "Balancer" });
          }
        }
      }
    } catch (e) {
...
// ================================================
// --- SEQUENTIAL BROADCAST FOR MULTIPLE POOLS ---
export async function broadcastSequentially(pools) {
  for (const pool of pools) {
    try {
      const result = await broadcastPendingPool(pool);
      console.log(result); // { status, txHash }
    } catch (e) {
      console.error(`[broadcast] Pool ${pool.id} failed:`, e.message);
    }
    await new Promise(r => setTimeout(r, 10)); // optional 10 ms gap to avoid nonce collision
  }
}