...
// --- retry wrapper (with provider rotation + limiter + optional timeout) ---
...
async function safeCall(fn, retries = 3, delayMs = 1000, timeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      return await withLimit(() => withTimeout(fn(), timeoutMs, "rpc_timeout"));
    } catch (err) {
      lastErr = err;
      console.warn(`[RPC ERROR] attempt ${i + 1}: ${err?.message || err}`);
      try {
        provider = rotateProvider(`validate-configs attempt ${i + 1}`);
      } catch {}
      if (i < retries - 1) {
...
async function validateRouter(addr) {
  ...
  // If factory() returns a valid address, treat as v2-compatible router
  if (factory && isAddr(factory)) return { ok: true, reason: "v2" };

  // Otherwise check code exists (could be non-standard router lacking factory())
