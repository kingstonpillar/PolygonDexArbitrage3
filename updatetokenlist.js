...
const OUT_FILE = path.resolve("./tokenlist.json"); // keep cwd-based output
...
async function reinitProvider() {
  provider = patchProviderBlockAPI(rotateProvider("updatetokenlist:retry"));
}
...
async function getBlockCompat(tagOrNumber, withTxs = false) {
  return withRotatingProvider(async () => {
    ...
    return withTxs ? p.getBlockWithTransactions(tagOrNumber) : p.getBlock(tagOrNumber);
  });
}
...
function dedupeMerge(baseList, newList) {
  ...
  for (const t of newList) {
    ...
    byAddr.set(k, {
      address: k, // keep lower-case logic as-is
