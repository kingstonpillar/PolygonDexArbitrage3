...
async function ensureProviderHealthy() {
  for (let i = 0; i < 4; i++) {
    ...
    provider = getReadProvider();  // Replaced rotateProvider with getReadProvider
    await sleep(200 * (i + 1));
  }
  return false;
}
...
// y(log); setImmediate(processNext);
        }
      }
      lastScanned = to;
    } catch (e) {
...
function startSwapPollV2(pairAddrsLower, poolsByAddr, poolsByPairKey, poolsByToken, edgeThreshold = 0, opts = {}) {
  ...
  const tick = async () => {
    if (stopped) return;
    try {
      let head; try { head = await provider.getBlockNumber(); } catch { provider = getReadProvider(); await sleep(200); head = await provider.getBlockNumber(); }
      if (!Number.isFinite(Number(head))) return;
      ...
      for (let start = from; start <= to; start += maxRange) {
        ...
        for (const addrChunk of batches(pairAddrsLower, ADDR_BATCH)) {
          ...
          try { logs = await safeGetLogs(filter); }
            ...
            provider = getReadProvider(); await sleep(150 + Math.floor(Math.random() * 250));
