...
async function ensureProviderHealthy() {
  for (let i = 0; i < 4; i++) {
    ...
    provider = getReadProvider(); // rotate to a fresh provider
    await sleep(200 * (i + 1));
  }
  return false;
}
...
async function discoverByLogs() {
  ...
  catch { provider = getReadProvider(); head = await provider.getBlockNumber(); }
...
function startSwapPollBalancer(vaultAddr, poolIdToAddr, poolsByAddr, poolsByPairKey, poolsByToken, edgeThreshold = 0, opts = {}) {
  ...
  const tick = async () => {
    ...
    try {
      ...
      catch { provider = getReadProvider(); await sleep(200); head = await provider.getBlockNumber(); }
      if (!Number.isFinite(Number(head))) return;
