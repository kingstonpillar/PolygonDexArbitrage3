// dataprovider.js — full ethers v6, adaptive limiter, quota-aware, stable RPC, v6-compatible
...
function queueDepth() {
return fastWaiters.length + waiters.length + Math.max(active, 0);
}
...
const next = WRITE_RPC_URLS[wIdx];
try {
...
probes.map((p) => isHealthy(p).then((ok) => ({ ok, p })).catch(() => ({ ok: false, p })))
);

const winner = checks.find((c) => c.ok)?.p;
if (winner) {
// map winner back to its index and update the GLOBAL wrapped provider
...
return _ensureRead(); // always return the wrapped global provider
}
