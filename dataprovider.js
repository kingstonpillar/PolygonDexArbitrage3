...
/* ----------------------------
   Verify provider is on expected chain (clean production-safe)
---------------------------- */
export async function verifySameChain() {
  try {
    const provider = await getReadProvider();
    const network = await provider.getNetwork();
    const expectedChain = Number(POLYGON_CHAIN_ID);
    ...
    if (actualChain !== expectedChain) {
      ...
      return false;// dataprovider.js — single RPC, auto-recovery, leading-zero hex fix
import 'dotenv/config';
import { ethers } from 'ethers';
import { WRITE_RPC_URL, POLYGON_CHAIN_ID } from './rpcConfig.js'; // now import chainId
