...
// Convert BigInt or numeric strings to float
...
// Fetch live token USD price (cached)
async function getLiveTokenPrice(symbol) {
  const now = Date.now();
  if (_tokenPrices[symbol] && now - _lastUpdate[symbol] < 60_000) {
    return _tokenPrices[symbol];
  }

  const price = await getTokenUsdPrice(symbol);
  _tokenPrices[symbol] = price || 0;
  _lastUpdate[symbol] = now;
  return price;
}
...
export async function calcLiquidityUSD(reserveAmount, tokenAddress, priceMap = {}) {
  ...
  if (HUB_TOKENS.slice(0, 3).includes(addr)) {
    return bnToFloat(reserveAmount, 6);
  }
  ...
  // Volatile tokens
  if (priceMap[addr]) {
    return bnToFloat(reserveAmount, 18) * priceMap[addr];
  }

  // Fallback (assume MATIC correlation)
