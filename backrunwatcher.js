...
export async function startBackrunWatcher() {
  ...
  socket.on("open", () => {
    console.log("✅ Connected to pendingTransaction.js feed (ws://127.0.0.1:7001)");
  });

  socket.on("message", async (data) => {
    try {
      const tx = JSON.parse(data.toString());
      if (!tx?.hash || !tx?.to || !markTxSeen(tx.hash)) return;
      ...
      await limit(async () => {
        ...
        const routerKind = liveDexData.kind;
        const decoded = tx.decoded || {};

        if (decoded.amountIn || decoded.amountOutMin) {
          console.log(
            `🔍 ${liveDexName} tx ${tx.hash.slice(0, 10)} | in=${decoded.amountIn || "?"} outMin=${decoded.amountOutMin || "?"}`
          );
        }
        ...
        const impacts = await Promise.all(
          livePairs.map((livePair) => {
            ...
            const amountIn = decoded.amountIn
              ? BigInt(decoded.amountIn)
              : ethers.parseUnits("100", decimals0);
          ...
          if (bestCatch) {
            watcher.emit("arbOpportunity", {
              ...
              live: {
                ...
                tokenIn: livePair.token0,
                tokenOut: livePair.token1,
                amountIn: decoded.amountIn ? decoded.amountIn.toString() : "N/A",
                amountOutMin: decoded.amountOutMin ? decoded.amountOutMin.toString() : "N/A",
      ...
      console.error("❌ Error in WebSocket message:", err.message);
    }
  });

  socket.on("close", () => {
    console.warn("⚠️ WebSocket closed. Reconnect manually or restart PM2 process.");
  });
