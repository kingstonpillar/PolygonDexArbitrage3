...
export async function startBackrunWatcher() {
  ...
  provider.on("pending", async (txHash) => {
    ...
    await limit(async () => {
      try {
        ...
        // === Step 2: Process each pair concurrently ===
        await Promise.all(
          livePairs.map(async (livePair) => {
            ...
            if (liquidityUSD < 30_000) return;

            // === Step 3: Check all other Dexes (cached) ===
  ...
  console.log("[Watcher] ⚡ Fast mode active — listening for pending transactions...");
}