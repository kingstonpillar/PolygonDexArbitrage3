...
async function readFailover(reason = "readCall_error") {
  ...
  await new Promise((r) =>
    setTimeout(r, 120 + Math.floor(Math.random() * 160))
  );
}
...
async function readCall(label, fn, timeoutMs) {
  ...
  try {
    ...
    return await withTimeout(
      safeRpcCall(() => fn(p2)), // ✅ limiter on retry
      timeoutMs ?? 1500,
...
export const reserveTradeCheck = async ({
  ...
  try {
    ...
    if (poolType === "V2") {
      // Use readCallWithProvider to benefit from limiter+failover (logic same)
    ...
    if (poolType === "V3") {
      // Use readCallWithProvider for consistency (logic same)
      const out = await readCallWithProvider(
