  ...
  const onBlock = async (blockNumber) => {
    ...
    try {
      ...
      if (/(rate|limit|timeout|429|temporarily unavailable|ECONNRESET|ETIMEDOUT)/i.test(String(e?.message))) {
        try {
          // Attempt to get a fresh live provider (auto-rotates internally if current fails)
          baseProvider = await getReadProvider();
          await ensurePolygonNetwork(baseProvider).catch(() => {});
          activeProvider = baseProvider;

          // Reattach block listener
          activeProvider.on?.('block', onBlock);
          console.log('[PROVIDER] Refreshed READ provider & reattached block listener');
        } catch (e2) {
          console.warn('[PROVIDER] Failed to refresh READ provider:', e2?.message || e2);
        }
      }
    }
  };
...
async function processTransactions() {
  ...
  try {
    ...
    for (const pool of allPools) {
      ...
      // Get the loan token decimals dynamically
    const loanDec = await getTokenDecimals(baseProvider, pool.loanAsset);
      ...
      const minOut = calcMinOut(pool.amount); // BigInt
       const expectedOutWei = (minOut * 10000n) / BigInt(10000 - MAX_SLIPPAGE_BPS);
...
// ===========================================================
   function calcMinOut(amount, decimals = 18) {
  if (!amount) return 0n;
...
// ===========================================================
//  11   Private RPC broadcast with fallback (Logic 11 replacement)
// Uses only private RPCs; integrates safeRpcCall
// ===========================================================
...
const rpcState = PRIVATE_RPCS.map(() => ({ failCount: 0, lastUsed: 0 }));
...
async function sendWithRpcFallback(txReq, nonce) {
  ...
  for (let i = 0; i < PRIVATE_RPCS.length; i++) {
    // Smart rotation: pick the RPC with least failCount / oldest usage
    const idx = rpcState
      .map((s, i) => ({ ...s, idx: i }))
      .sort((a, b) => a.failCount - b.failCount || a.lastUsed - b.lastUsed)[0].idx;

    const rpc = PRIVATE_RPCS[idx];
    rpcState[idx].lastUsed = Date.now();

    try {
      const prov = new ethers.JsonRpcProvider(rpc.url, { name: 'polygon', chainId: CHAIN_ID });
      const txHash = await safeRpcCall(() => prov.send('eth_sendRawTransaction', [signedRaw]));

      console.log(`🚀 Sent via ${rpc.name}: ${txHash}`);
      rpcState[idx].failCount = 0; // reset fail count on success
      ...
      lastError = e;
      rpcState[idx].failCount++; // increment fail count
      console.warn(`⚠️ ${rpc.name} send failed: ${e?.message || e}`);
    }
  }
